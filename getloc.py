...
class Solution():
    ...
    def query_player(self):
        ...
        (name, self.playerx, self.playery) = res.fetchone()
        print(f'player name: {name} at location: {self.playerx}x{self.playery}')
        url = dist.url(self.playerx, self.playery)
    ...
    def query_vehicles(self):
        ...
        for row in records:
            d = int(dist.dist(self.playerx, self.playery, row[0], row[1]))
            self.vehicles.append((row[0], row[1]))
            self.all_vehicles[d] = row

    def dump_vehicles(self):
        ''' dump location to file, sort by distance to player's location '''

        with open(self.vehicletxt, "wt", encoding='utf-8') as f:
            print("# generated by getloc.py...\n", file=f)
            print("_PLAYER = ", f'({self.playerx}, {self.playery})\n', file=f)
            ...
            for k in sorted(self.all_vehicles.keys()):
                v = self.all_vehicles[k]
                ln = f'({v[0]}, {v[1]}),'
                print(ln, file=f)
            print("]", file=f)
        size_vehicles = len(self.vehicles)
        print(f'dump {size_vehicles} vehicles to {self.vehicletxt}')
